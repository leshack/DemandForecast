...
def forecast_app(): 
    ...
    def load_and_preprocess_data(csv_path):
        ...
        # df = df[df['Invoice Date'] >= start_date]
    ...
    def aggregate_data(df):
        monthly_sales = df.groupby(['Month', 'Item Description', 'Colour Group', 'Classification'])[['Value', 'Quantity']].sum().reset_index()
    ...
    def forecast_sales_and_quantities(monthly_sales, classification, item_description, colour_group, forecast_period, product_level, forecast_model):
        if product_level:
            group_data = monthly_sales[
                (monthly_sales['Item Description'] == item_description) &
                (monthly_sales['Classification'] == classification)
            ].groupby('Month').sum().reset_index()
        else:
            group_data = monthly_sales[
                (monthly_sales['Item Description'] == item_description) &
                (monthly_sales['Colour Group'] == colour_group) &
                (monthly_sales['Classification'] == classification)
            ]
        ...
        prophet_data_sales = group_data[['Month', 'Value']].rename(columns={'Month': 'ds', 'Value': 'y'})
        prophet_data_quantity = group_data[['Month', 'Quantity']].rename(columns={'Month': 'ds', 'Quantity': 'y'})

        if forecast_model == 'Prophet':
            model_sales = Prophet()
            model_sales.fit(prophet_data_sales)
            future_sales = model_sales.make_future_dataframe(periods=forecast_period, freq='MS')
            forecast_sales = model_sales.predict(future_sales)

            model_quantity = Prophet()
            model_quantity.fit(prophet_data_quantity)
            future_quantity = model_quantity.make_future_dataframe(periods=forecast_period, freq='MS')
            ...
            ax.plot(group_data['Month'], group_data['Value'], label='Actual Sales', color='blue', marker="o")
            ...
            future_dates = pd.date_range(start=group_data['Month'].max(), periods=forecast_period+1, freq='MS')[1:]
            ...
            ax.plot(forecast_sales_continuous['ds'], forecast_sales_continuous['yhat'], label='Forecast Sales', color='red', marker="o")
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.plot(group_data['Month'], group_data['Quantity'], label='Actual Quantity', color='blue', marker="o")
            ax.plot(forecast_quantity_continuous['ds'], forecast_quantity_continuous['yhat'], label='Forecast Quantity', color='red', marker="o")
            ax.set_title(f'Quantity Forecast for {item_description} {"(All Colors)" if product_level else f"({colour_group})"}')
            ax.set_xlabel('Date')
            ax.set_ylabel('Quantity')
            ax.xaxis.set_major_formatter(plt.matplotlib.dates.DateFormatter('%Y-%b'))
        ...
        #     group_data['Month No.'] = group_data['Month'].dt.month
        #     group_data['Year'] = group_data['Month'].dt.year

        #     X = group_data[['Month No.', 'Year', 'Value', 'Quantity']]
    ...
    def forecast_overall_sales_and_quantities(monthly_sales, forecast_period, forecast_model):
        overall_data = monthly_sales.groupby('Month').sum().reset_index()
        
        if overall_data.dropna().shape[0] < 2:
            st.write("Not enough data for overall forecasting.")
            return
        
        prophet_data_sales = overall_data[['Month', 'Value']].rename(columns={'Month': 'ds', 'Value': 'y'})
        prophet_data_quantity = overall_data[['Month', 'Quantity']].rename(columns={'Month': 'ds', 'Quantity': 'y'})
        ...
        if forecast_model == 'Prophet':
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.plot(overall_data['Month'], overall_data['Value'], label='Actual Quantity', color='blue', marker="o")
            ax.plot(forecast_sales['ds'], forecast_sales['yhat'], label='Forecast Sales', color='red')
            ax.fill_between(forecast_sales['ds'], forecast_sales['yhat_lower'], forecast_sales['yhat_upper'], alpha=0.2)
            ...
            st.markdown("""
                In addition to sales, we also forecast the quantities of products needed for each month. This helps us determine 
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.plot(overall_data['Month'], overall_data['Quantity'], label='Actual Quantity', color='blue', marker="o")
            ax.plot(forecast_quantity['ds'], forecast_quantity['yhat'], label='Forecast Quantity', color='red')
            ax.fill_between(forecast_quantity['ds'], forecast_quantity['yhat_lower'], forecast_quantity['yhat_upper'], alpha=0.2)
            ...
            future_dates = pd.date_range(start=overall_data['Month'].max(), periods=forecast_period+1, freq='MS')[1:]
            ...
            # Apply SARIMA for overall sales and quantity
            ...
            # Add last actual sales and quantity data
            last_actual_sales = pd.DataFrame({
                'ds': [overall_data['Month'].max()],
                'yhat': [overall_data['Value'].iloc[-1]]
            })
        ...
        #         # Prepare features for forecasting
        #         overall_data['Month No.'] = overall_data['Month'].dt.month
        #         overall_data['Year'] = overall_data['Month'].dt.year
    
        #         X = overall_data[['Month No.', 'Year', 'Value', 'Quantity']]
        #         X_scaled = scaler.transform(X)
    ...
    if csv_path is not None:
        ...
        forecast_period = st.sidebar.slider('Forecast Period (months):', min_value=1, max_value=24, value=12)
    ...
    if st.sidebar.button('Generate Forecast'):
        forecast_sales_and_quantities(monthly_sales, classification, item_description, colour_group, forecast_period, product_level, forecast_model)
        forecast_overall_sales_and_quantities(monthly_sales, forecast_period, forecast_model)
